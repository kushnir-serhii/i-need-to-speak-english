...
- [x] **Slice 3: Wire streaming to frontend + error handling + final build**
  - [x] Add `appendChunk(id, chunk)` and `finalizeMessage(id)` actions to `src/store/useChatStore.ts`. **[Agent: nextjs-frontend]**
  - [x] Update `ChatInput.tsx` — on submit: call `addMessage('user', input)`, then `addMessage('assistant', '')` (captures its `id`), then `setStreaming(true)`; `POST /api/chat` with the full message list and `visitorId` from `useUserStore.getState()`; loop `response.body.getReader()` calling `appendChunk(id, chunk)` per chunk; on stream end call `finalizeMessage(id)`; on `catch` call `finalizeMessage(id)` and fire `useNotification().toast('error', 'Something went wrong. Please try again.')`. **[Agent: nextjs-frontend]**
  - [x] Update `ChatThread.tsx` — for each message: if `role === 'assistant' && isStreaming && content === ''` render `<TypingIndicator />`; otherwise render `<MessageBubble />`; add auto-scroll `useEffect` with `messages` as dependency calling `bottomRef.current?.scrollIntoView({ behavior: 'smooth' })`. **[Agent: nextjs-frontend]**
