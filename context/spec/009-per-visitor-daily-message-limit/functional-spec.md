...
**Acceptance Criteria:**
- [ ] When a visitor opens the chat in shared-key mode and has not yet reached the daily limit, the chat header displays a remaining count (e.g., "12 messages left today").
- [ ] Each time the visitor sends a message and receives an AI reply, the number in the header decreases by one.
- [ ] When the visitor switches to their own key, the remaining message counter disappears from the header.
...
**Acceptance Criteria:**
- [ ] When the visitor's last allowed message receives an AI reply, the input field immediately becomes greyed out and cannot be clicked or typed into.
- [ ] The send button is visually disabled and cannot be clicked.
- [ ] The popup "You've reached today's limit. Come back tomorrow! You can also use your own key to continue." appears after the final reply is received.
- [ ] Dismissing the popup does not restore the input — it remains blocked.
- [ ] The remaining counter in the header shows "0 messages left today" (or equivalent) while blocked.
...
**Acceptance Criteria:**
- [ ] If the app is open in the blocked state and the day changes, the input field automatically becomes active again without any action from the visitor.
- [ ] The header counter resets to the full daily allowance after the reset occurs.
- [ ] If a visitor who was blocked the previous day returns the next day in a new session, the input is immediately available and the header shows the full remaining count.
...
**Acceptance Criteria:**
- [ ] When a visitor is using their own key, no remaining message counter appears in the chat header.
- [ ] The message input is never greyed out or disabled due to a daily limit when the visitor is on their own key.
